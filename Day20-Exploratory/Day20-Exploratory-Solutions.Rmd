...
bus_breakdowns_cleaned %>%
  ggplot(aes(x = Reason, fill = School_Year)) + 
...
```{r}
bus_breakdowns_cleaned %>%
  filter(Reason == "Mechanical Problem") %>%
  group_by(Bus_Company_Name) %>%
  summarize(Count = n(), 
            Students_Impacted = sum(Number_Of_Students_On_The_Bus)) %>%
  arrange(desc(Count)) %>%
  top_n(10) %>%
  ggplot(aes(x = Bus_Company_Name, y = Count)) +
  geom_col() +
  coord_flip()
```
...
```{r}
bus_breakdowns_cleaned %>%
  filter(Reason == "Heavy Traffic" & School_Year == "2020-2021") %>%
  group_by(Hour = hour(Occurred_On), Boro) %>%
  summarize(Count = n()) %>%
