...
ct_school_attendance %>%
  ...
  select(reportingdistrictcode, 
         reportingdistrictname, 
         attrate_202021, 
         percent_students_fully_remote)
```
...
```{r}
ct_school_attendance %>%
  left_join(ct_school_learning_model, 
            by = c("reportingdistrictcode" = "district_code")) %>%
  select(reportingdistrictcode, 
         reportingdistrictname, 
         attrate_202021, 
...
ct_school_attendance %>%
  ...
  select(reportingdistrictcode, reportingdistrictname, attrate_202021, percent_students_fully_remote)
```
...
```{r}
ct_school_attendance %>%
  right_join(ct_school_learning_model, 
            by = c("reportingdistrictcode" = "district_code")) %>%
  select(reportingdistrictcode, 
         reportingdistrictname, 
         attrate_202021, 
...
```{r}
ct_school_attendance %>%
  full_join(ct_school_learning_model, by = c("reportingdistrictcode" = "district_code")) %>%
  select(reportingdistrictcode, reportingdistrictname, attrate_202021, percent_students_fully_remote)
```
...
```{r}
ct_school_attendance %>%
  full_join(ct_school_learning_model, 
            by = c("reportingdistrictcode" = "district_code")) %>%
  select(reportingdistrictcode, 
         reportingdistrictname, 
         attrate_202021, 
         percent_students_fully_remote) %>%
  filter(is.na(reportingdistrictname) | is.na(percent_students_fully_remote))
```
...
```{r}
ct_school_learning_model_cleaned <- 
  ct_school_learning_model %>%
  mutate(percent_students_fully_remote = na_if(percent_students_fully_remote, "*")) %>%
  mutate(percent_students_fully_remote = str_replace(percent_students_fully_remote, "%", "")) %>%
  mutate(percent_students_fully_remote = as.integer(percent_students_fully_remote)) %>%
  mutate(percent_students_fully_remote = percent_students_fully_remote / 100)

ct_school_attendance %>%
  inner_join(ct_school_learning_model_cleaned, 
            by = c("reportingdistrictcode" = "district_code")) %>%
  select(reportingdistrictcode, 
         reportingdistrictname, 
         attrate_202021, 
         percent_students_fully_remote) %>%
  ggplot(aes(x = attrate_202021, y = percent_students_fully_remote)) +
