...
## Checkpoint 1: Check to see whether the values in your key are the same type. 

If not, convert the `reportingdistrictcode` column in `ct_school_attendance` to an integer format. Store the resulting data frame in `ct_school_attendance` 

> Note: An update the data producers made to the dataset *while I was writing this lab*, rendered this step unnecessary. Let's make sure it remains unnecessary when we are in class together tomorrow. 

```{r}
#Code here!
```

## Checkpoint 2: Inner Join
